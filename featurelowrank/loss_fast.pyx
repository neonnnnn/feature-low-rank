...
cdef class SoftCrossEntropy(LossFunction):
    def __init__(self):
        self.mu = 0.25

    cdef double loss(self, double p, double y):
        """ Cross entropy loss: - y * log(sigm(p)) - (1-y) * log(1-sigm(p)) """
        # = y*log(1+exp(-p)) - (1-y)*(-p-log(1+exp(-p)))
        # = (1-y)*p + log(1+exp(-p))
        # log(1+x) \simeq x (if x is small)
        if p > 18:
            return exp(-p) + (1-y)*p
        elif p < -18.0:
            return -y*p
        return log(1.0+exp(-p)) + (1-y)*p
